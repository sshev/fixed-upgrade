/**
 * @license Angular v4.2.2-72687eb
 ...
 * @stable
 */
var VERSION = new _angular_core.Version('4.2.2-72687eb');
/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
        ...
        var initModule = module$1(INIT_MODULE_NAME, [])
            ...
            function ($injector) {
                ...
                setTimeout(function () {
                    ...
                    var lastRun = window.performance.now();
                    var isDigestScheduled = false;
                    var digestsPerFrame = 0;
                    var FRAME_DURATION = 16; // 16ms - typical frame duration for 60FPS rendering
                    var runDigest = function () {
                        isDigestScheduled = false;
                        $rootScope.$$phase ? $rootScope.$applyAsync() : $rootScope.$digest();
                    };
                    var subscription = _this.ngZone.onMicrotaskEmpty
                        .filter(function () { return !isDigestScheduled; })
                        .subscribe(function () {
